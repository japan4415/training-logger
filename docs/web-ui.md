...
- 約4.21MBのgzipモデル（筋150メッシュ＋全身表面）とThree.jsを同一オリジンで配信する。記録を外部ビューアーへ送信しない。WebGL/通信失敗時も対象筋の文字表示を保持する。
- Atlas未収録の広背筋・腹直筋は名称のみ表示し、別筋肉で代用しない。構造化した空設定も尊重する。未設定（NULL）の種目だけ、従来の自由記述を区切り文字で分割して参考表示する。
