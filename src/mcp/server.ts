...
■ 記録の運用
同日の log_workout 再呼び出しは追記です。修正は update_workout、削除は delete_workout を使ってください。ノート画像からの登録は、不明点を確認し下書きをユーザーに見せて承認を得てから log_workout を呼んでください（詳細な手順は Skill log-workout を参照）。ノート写真を保存したい場合は log_workout の後に create_photo_upload_link でリンクを案内する（画像本体は MCP では送れない）。
